...
fn apply_saved_image_defaults(
    ...
    if let Some(prefix) = resolve_string_setting(settings, settings_key, "positive_prompt") {
    ...
    request.negative_prompt = request
        .negative_prompt
        .take()
        .or_else(|| resolve_string_setting(settings, settings_key, "negative_prompt"));
    request.steps = request
        .steps
        .or_else(|| resolve_u32_setting(settings, settings_key, "steps"));
    request.cfg_scale = request
        .cfg_scale
        .or_else(|| resolve_f32_setting(settings, settings_key, "cfg_scale"));
    request.denoising_strength = request
        .denoising_strength
        .or_else(|| resolve_f32_setting(settings, settings_key, "denoising_strength"));
    request.width = request
        .width
        .or_else(|| resolve_u32_setting(settings, settings_key, "width"));
    request.height = request
        .height
        .or_else(|| resolve_u32_setting(settings, settings_key, "height"));
    request.sampler = request
        .sampler
        .take()
        .or_else(|| resolve_string_setting(settings, settings_key, "sampler"));
    request.seed = request
        .seed
        .or_else(|| resolve_i32_setting(settings, settings_key, "seed"));
    request.batch_size = request
        .batch_size
        .or_else(|| resolve_u32_setting(settings, settings_key, "batch_size"));
    request.scheduler = request
        .scheduler
        .take()
        .or_else(|| resolve_string_setting(settings, settings_key, "scheduler"));
    request.clip_skip = request
        .clip_skip
        .or_else(|| resolve_i32_setting(settings, settings_key, "clip_skip"));
}
...
pub(super) fn resolve_string_setting(
    ...
    resolve_setting_value(settings, settings_key, suffix).and_then(|value| {
...
pub(super) fn resolve_u32_setting(
    ...
    resolve_setting_value(settings, settings_key, suffix)
        .and_then(|value| value.parse::<u32>().ok())
}

fn resolve_i32_setting(settings: &AppSettings, settings_key: &str, suffix: &str) -> Option<i32> {
    resolve_setting_value(settings, settings_key, suffix)
...
fn resolve_setting_value(
    ...
    settings
        .extra_settings
        .get(&format!("{settings_key}_{suffix}"))
        .cloned()
}
