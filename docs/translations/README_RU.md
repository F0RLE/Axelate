<div align="center">
  <br />
  <img src="../../src-tauri/icons/icon.png" alt="Axelate Logo" width="160" height="160" />
  <br />

  <h1 style="border-bottom: none; margin-bottom: 0;">Axelate</h1>
  <p style="font-size: 1.1em; color: #888; font-style: italic;">Безопасная Среда для AI-Агентов Нового Поколения</p>

  <br />

  <!-- Primary Call to Action -->
  <a href="https://github.com/F0RLE/Axelate/releases">
    <img src="https://img.shields.io/badge/Скачать_Платформу-007AFF?style=for-the-badge&logo=windows&logoColor=white" height="40" alt="Download Now" />
  </a>

  <br />
  <br />

  <!-- Navigation Bar -->
  <p>
    <a href="../../README.md"><img src="https://img.shields.io/badge/English-31303a?style=for-the-badge&logo=google-translate&logoColor=white" height="30" alt="English"/></a>
    &nbsp;
    <a href="README_CN.md"><img src="https://img.shields.io/badge/Chinese-31303a?style=for-the-badge&logo=google-translate&logoColor=white" height="30" alt="Chinese"/></a>
    &nbsp;
    <a href="https://github.com/F0RLE/Axelate/releases"><img src="https://img.shields.io/badge/v0.1.4-31303a?style=for-the-badge&logo=semver&logoColor=white" height="30" alt="Version"/></a>
    &nbsp;
    <img src="https://img.shields.io/badge/Статус-Публичная_Бета-orange?style=for-the-badge" height="30" alt="Status: Beta"/>
    &nbsp;
    <a href="../ru/architecture.md"><img src="https://img.shields.io/badge/Документация-31303a?style=for-the-badge&logo=gitbook&logoColor=white" height="30" alt="Docs"/></a>
  </p>

  <br />
</div>

> [!IMPORTANT]
> **Axelate сейчас находится в стадии Публичного Бета-тестирования (v0.1.4).**
>
> Это предварительная версия программного обеспечения. Функционал может быть экспериментальным, а API могут измениться без уведомления. Стабильных релизов пока нет.

---

<div align="center">

## ✨ Будущее Уже Здесь

**Axelate** — это выделенное, аппаратно-защищенное пространство для ваших профессиональных AI-агентов.
<br>Создано для тех, кто не готов жертвовать **приватностью**, **скоростью** или **контролем**.

</div>

<br>

<div align="center">

| 🛡️ Привязка к Железу | ⚡ Нативная Скорость | 🧩 Изолированные Модули |
| :---: | :---: | :---: |
| Ваши данные шифруются ключами **AES-256-GCM**, привязанными к серийному номеру материнской платы. Защита от кражи. | Гибридное ядро **Rust Kernel** + **V8 Shell**. Нулевая задержка, мгновенный запуск и минимальное потребление ресурсов. | Запускайте агентов кодинга, аналитиков данных и творческие инструменты в строгой изоляции. Никакого перекрестного доступа. |

</div>

---

<h2 align="center">🚀 Начало Работы</h2>

<div align="center">

<p>
  <b>1. Скачайте</b> установщик со <a href="https://github.com/F0RLE/Axelate/releases">Страницы Релизов</a>.
  <br>
  <b>2. Запустите</b> `Axelate Setup.exe`.
  <br>
  <b>3. Инициализируйте</b> ваше защищенное хранилище и начните установку модулей.
</p>

</div>

---

<h2 align="center">👩‍💻 Для Разработчиков</h2>

Axelate построен на архитектуре "Pass-Through", дающей фронтенд-модулям прямой доступ к системным сервисам Rust.

<div align="center">

[![Developer Cookbook](https://img.shields.io/badge/📖_Руководство_Разработчика-Читать-31303a?style=flat-square)](../ru/development.md)
[![Architecture Spec](https://img.shields.io/badge/🏗️_Спецификация_Архитектуры-Изучить-31303a?style=flat-square)](../ru/architecture.md)
[![Contributing](https://img.shields.io/badge/🤝_Гайдлайн_Вклада-Читать-31303a?style=flat-square)](../../CONTRIBUTING.md)

</div>

---

<div align="center">

<br>

  <a href="https://github.com/F0RLE/Axelate/issues"><img src="https://img.shields.io/badge/Сообщить_о_Баге-31303a?style=for-the-badge&logo=github&logoColor=white" height="30" alt="Report Bug" /></a>
  &nbsp;
  <a href="https://github.com/F0RLE/Axelate/issues"><img src="https://img.shields.io/badge/Запросить_Фичу-31303a?style=for-the-badge&logo=github&logoColor=white" height="30" alt="Request Feature" /></a>
  &nbsp;
  <a href="../../SECURITY.md"><img src="https://img.shields.io/badge/Политика_Безопасности-31303a?style=for-the-badge&logo=github&logoColor=white" height="30" alt="Security Policy" /></a>

<br>
<br>

<img src="https://img.shields.io/badge/Сделано_с_❤️_командой_Axelate-31303a?style=flat-square" alt="Made with Love" />

<sub>Copyright © 2026 Axelate. All Rights Reserved.</sub>

</div>
