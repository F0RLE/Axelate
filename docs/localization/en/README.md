# Axelate English Documentation

English documentation is the canonical reference for Axelate.

## Current Behavior

- [Current State](CURRENT_STATE.md) - repository-grounded product snapshot
- [User Guide](USER_GUIDE.md) - current desktop app usage
- [Getting Started](GETTING_STARTED.md) - source setup and first run
- [Development Workflow](DEVELOPMENT_WORKFLOW.md) - daily contributor workflow
- [Architecture](ARCHITECTURE.md) - frontend, backend, runtime, and contract map
- [Trust Model](TRUST_MODEL.md) - current and future trust boundaries
- [Releases](RELEASES.md) - release workflow and tag rules

## Integrations

- [Integration Development](INTEGRATION_DEVELOPMENT.md) - build local integrations
- [Integration API](INTEGRATION_API.md) - local HTTP API contract and future
  agent-control base
- [Custom Integrations](CUSTOM_INTEGRATIONS.md) - manifest and import rules

## Planning

- [Vision](VISION.md) - product direction
- [Roadmap](ROADMAP.md) - ordered execution plan

Planning docs are not setup guides and do not mean every listed feature exists
today.
